...
What is not built yet answers 501 rather than 404 on purpose, for any method and for every path under the prefix including the prefix itself, because a 404 reads as a wrong url and sends somebody looking for a typo they did not make. The body carries a link to the checklist that piece is a box on, so the next question, whether this is nearly done or has not been started, is one url away rather than a search. An endpoint under `/auth/v1` that is not served yet answers the same way. Anything outside all four prefixes is a 404 in the words the hosted gateway uses, `no Route matched with those values`, so a client that gets one is looking at the same sentence it would get from Supabase.
