...
A phase that was made to do work twice says so on a trouble line, the retries split into throttling, server errors, transport errors and the ones that ran out of attempts, plus the CAS conflicts and the calls that failed outright, and a phase that put anything says how many commits rode one PUT, which is the store bill divided by the work done and the number group commit is supposed to move.
...
And what it would have cost on a bucket, on the cost line, which is those same counters times a published price card and nothing else: `zou cost <file> --since <earlier>` prints it for a finished run, `--list-cards` names the cards, and `ZOU_BENCH_CARD` picks a different one for a whole run.
A tps says which leg was faster and that line says which leg was cheaper, and on object storage the two questions have different enough answers to be worth printing side by side.
...
The same sampler reads the disk counters for the devices under the run, bytes and IOPS and await and utilization, and each phase boundary measures what the run occupies rather than what it moved: the data directory, the logical size of the database, and the store broken into its wal tail, its page objects, its checkpoints and its layers.
Write amplification is device bytes over the wal bytes postgres says it produced and space amplification is the store over the logical size, both derived from those measurements rather than asserted, and both answerable on the vanilla leg too so the ratio means something next to it.
The compression ratio behind that store figure is counted inside the store, raw bytes handed to the compressor against bytes actually written, per compressor, because the objects on the bucket are the compressed ones and a block that did not compress went out raw with nothing on it to say so.
...
That tail is then broken down statement by statement, so a phase says how much of a transaction was the work and how much was the commit, and next to it the store prints the same commit wait measured from inside the engine, its tail plus the split across the window, the dispatch, the store call and the ack, because a commit that is slow because the batch waited and a commit that is slow because the PUT waited are the same number until something separates them.
