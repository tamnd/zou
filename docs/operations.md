...
They do not add up to a cycle, because the reader asks for the next batch before it hands over the last one and the tap runs alongside the selection and the sending.
A cycle is the larger of the tap and those two together, plus the decode, and which of the two is larger is the reading worth taking: tap larger says the database is the limit and the fan out is waiting on it, the other way round says the fan out is and the database is idle in between.
