...
`/realtime/v1` is served as far as the socket and broadcast go, which is described in [realtime.md](realtime.md). A client connects, joins channels, refreshes its token on a live connection and broadcasts to the other members of a topic. Presence, `postgres_changes` and private channels are not built, and a join asking for one of them is answered with an error naming what is missing rather than joined and left silent, because a channel that says SUBSCRIBED and never delivers anything is harder to debug than one that failed. The rest of the prefix, the REST broadcast endpoint among it, is a route in the router with nothing behind it.
