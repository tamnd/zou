...
`/realtime/v1` is served as far as the socket, broadcast and presence go, which is described in [realtime.md](realtime.md). A client connects, joins channels, refreshes its token on a live connection, broadcasts to the other members of a topic and tracks itself on one, and a socket joining a topic is told who is already there. A room can also be sent to over http, both the batch shape `channel.send()` falls back to and the single message shape `channel.httpSend()` posts, which is how a trigger or a worker talks to one. `postgres_changes` and private channels are not built, and a join asking for one of them is answered with an error naming what is missing rather than joined and left silent, because a channel that says SUBSCRIBED and never delivers anything is harder to debug than one that failed. The rest of the prefix is a route in the router with nothing behind it.
