...
| --- | ---: | ---: | ---: | --- |
| rest | 99 | 104 | 95% | the hand written suite, the surface a Supabase project actually uses; the five known differences are the wording of a parse refusal in corners of the grammar |
| postgrest | 1317 | 1317 | 100% | upstream's own test corpus turned into questions, derived from its spec files |
| auth | 168 | 176 | 95% | the known differences are below |
...
Everything in that state answers 501 rather than 404, so a client gets told the surface exists and is not finished instead of being told the url is wrong. The body names the surface and links the checklist the missing piece is a box on, which is [#190](https://github.com/tamnd/zou/issues/190) for storage and [#303](https://github.com/tamnd/zou/issues/303) for realtime, so somebody who hits one can go and see whether their case is nearly done or has not been started.
...
The time claims are read the way the server a caller thinks it is talking to reads them, because the two references disagree. PostgREST verifies with jose, which checks `exp`, `nbf` and `iat` and allows thirty seconds either side of each: a token half a minute past its expiry is still answered 200, and one issued half a minute from now is still a token. GoTrue verifies with golang-jwt, which checks `exp` and `nbf` with no leeway at all and never looks at `iat`. So a token ten seconds past `exp` is answered under `/rest/v1` and refused under `/auth/v1`, and that is upstream's behaviour rather than a zou decision. The refusals carry each reference's own wording: 401 `PGRST303` with `JWT expired`, `JWT not yet valid` or `JWT issued at future` on the rest surface, and 403 `bad_jwt` with `invalid JWT: unable to parse or verify signature, token has invalid claims: token is expired` on the auth one.

`aud` is not a signature time check on either surface, and that matches both references. PostgREST reads it only when an instance is configured with `jwt-aud`, and the reference stack configures none. GoTrue does not compare it in the middleware either: it works out the audience a request is for from the `x-jwt-aud` header, then from the token's own `aud`, then from its default of `authenticated`, and only the handlers that read a person compare the two. So a token addressed to `somebody-else` is answered, because it names the audience it is then checked against, while a token carrying no `aud` at all is turned away by `/auth/v1/user` with 400 `validation_failed` and `Token audience doesn't match request audience`, and so is one whose `aud` disagrees with an `x-jwt-aud` header. zou does the same, in `auth::requested_aud`.

`crates/zou-server/tests/attack.rs` pins all of it so the day any of it changes is a test edit rather than a surprise.
...
A table created in `public` is readable and writable by none of the three, which is what a project gets today: the default privileges the bootstrap sets are truncate, references, trigger and maintain, and a project grants the rest on the tables it means to expose. Nothing already in the database is swept, so a restored dump keeps the grants it arrived with. Row level security is still the app's job, because a granted table with no policy on it is open to whoever was granted it.
