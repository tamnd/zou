...
The `storage` suite is 163 cases: 32 over the bucket endpoints and 131 over the object ones, each asked with a service key, an anon key, and in a good few cases with no key at all.
...
What the object cases do not ask about yet: resumable uploads, image transforms and the S3 protocol.
