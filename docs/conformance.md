...
The `postgrest` suite is 1217 cases against PostgREST 14.15, and zou passes 704 of them, 57%, with 513 known differences.
That number is the honest one, and it is meant to be uncomfortable.
The suite asks everything upstream asks itself, including the parts of PostgREST nobody using Supabase has ever typed, so 57% against it and 92% against the hand written suite are both true and they measure different things.
The gap is a checklist in [tamnd/zou#125](https://github.com/tamnd/zou/issues/125), grouped by what has to be built rather than by the endpoint it shows up on, and it is a small number of missing features rather than 513 separate bugs: relationship resolution that answers `PGRST200`, spread embeds, function overload resolution, the parts of the query parser that answer `PGRST100`, `Prefer` handling, `Range` on a table, the media type handlers, and `explain`.
