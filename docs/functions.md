...
- `process` is a global as well as a module, the way it is in upstream's runtime, so a package that sets something on one sees it through the other. `env` is the function's environment and can be written to, though a write stays in the isolate that made it and never reaches the node, `stdout` and `stderr` go to the log, `nextTick` is a microtask, and `chdir` and `exit` throw rather than pretending.
