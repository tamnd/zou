...
Supabase compatibility is measured rather than claimed. `conformance/` asks the real PostgREST and GoTrue binaries and zou the same questions, and compares status, headers, and bodies. There are two REST suites: a hand written one about the surface a Supabase project uses, and a much larger one derived from PostgREST's own spec files. Both pass in full. The auth suite passes but for a few answers that differ on purpose, each of which is written down. The storage suite is asked against the storage-api image a local Supabase project runs, buckets, objects, image transforms and the S3 protocol, and it passes in full. supabase-js's own integration tests are run against zou as well, unedited apart from the url they point at, as are storage-js's own, and what either of them skips it skips on the scoreboard where a reader can see it. Resumable uploads and presence are asked in a third shape, because an upload and a room are conversations rather than requests: the real tus-js-client drives an upload against zou and supabase-js drives a presence channel and both ways of broadcasting over http, the answers are read back through the same client an application would use, and both files are run against a real supabase project so that the assertions are the reference's behaviour rather than ours. The questions and the recorded answers live in [tamnd/zou-conformance](https://github.com/tamnd/zou-conformance), pinned to a commit here, and every case that differs is listed with the reason it differs, see [docs/conformance.md](docs/conformance.md). Every number, per suite and per endpoint and per feature, is in [docs/scoreboard.md](docs/scoreboard.md), which CI rewrites on every merge, and none of them is repeated here, because a count typed into a paragraph is a count that stops being true without anybody noticing. What a project will actually notice when it moves is in [docs/compatibility.md](docs/compatibility.md).
