-- Taken from supabase/postgres-meta src/lib/sql, so that what zou reads
-- out of the catalog is what postgres-meta reads, down to the privilege
-- checks and the null handling.
-- It is here as it stands, filter included: the generator wants every
-- schema that is not a system one, whatever was asked for on the
-- command line.
