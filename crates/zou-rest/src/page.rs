...
//!
//! `limit` and `offset` are plain non negative integers on the query
//! string, embed scoped variants like `posts.limit` differ only in
//! the key so this module only parses values. The `Range` header
//! carries `first-last` or the open ended `first-` in items, the
//! `Range-Unit` header naming the unit is the router's business. A
//! range that ends before it starts is an error here and a 416 at the
//! HTTP layer, per what PostgREST does.
...
/// Parse a limit value, a plain non negative integer.
pub fn parse_limit(value: &str) -> Result<u64, Error> {
    number(value, 0, value.len(), "limit")
}

/// Parse an offset value, a plain non negative integer.
pub fn parse_offset(value: &str) -> Result<u64, Error> {
    number(value, 0, value.len(), "offset")
}
...
mod tests {
    ...
    fn limits_and_offsets() {
        ...
        for bad in ["", "-1", "+1", "1.5", " 1", "1 ", "ten", "0x10"] {
